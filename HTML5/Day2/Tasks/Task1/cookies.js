...
function setCookie(cookieName, cookieValue, expiryDate = null) {
  ...
  if (expiryDate === null) {
    ...
    document.cookie =
      cookieName + "=" + cookieValue + ";expires=" + expiryDate.toUTCString();
  }
}
...
function deleteCookie(cookieName) {
  if (arguments.length != 1)
    throw new Error("deleteCookie must have only one param");
  if (typeof cookieName !== "string")
    throw new Error("you must pass string type only");
...
function Getlength() {
  if (arguments.length !== 0) throw new Error("Getlength must have no args");
